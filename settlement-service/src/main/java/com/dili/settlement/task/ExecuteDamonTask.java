...
public class ExecuteDamonTask extends DamonTask implements Callable<Boolean> {
    ...
    public ExecuteDamonTask(CallbackConfiguration callbackConfiguration, RetryRecordService retryRecordService, ExecutorService executorService) {
        ...
        while (true) {
            ...
            try {
                ...
                for (int i = 0; i < reboot; i++) {
                    executorService.submit(new ExecuteQueueTask(callbackConfiguration, retryRecordService));
                }
            } catch (Exception e) {
