...
INSERT INTO settle_order_link(`settle_order_id`, `type`, `url`)
	...
	case temp.business_type
		when 1 then
			CONCAT('https://pre.ia.diligrp.com:8381/leaseOrder/view.action?orderCode=', temp.order_code, '&businessType=', temp.business_type)
		when 2 then
			CONCAT('https://pre.ia.diligrp.com:8381/earnestOrder/view.action?orderCode=', temp.order_code, '&businessType=', temp.business_type)
		when 3 then
		   CONCAT('https://pre.ia.diligrp.com:8381/depositOrder/view.action?orderCode=', temp.order_code, '&businessType=', temp.business_type)
		end
...
INSERT INTO settle_order_link(`settle_order_id`, `type`, `url`)
	...
	case temp.business_type
		when 1 then
			CONCAT('https://pre.ia.diligrp.com:8381/refundOrder/view.action?orderCode=', temp.order_code, '&businessType=', temp.business_type)
		when 2 then
			CONCAT('https://pre.ia.diligrp.com:8381/refundOrder/view.action?orderCode=', temp.order_code, '&businessType=', temp.business_type)
		when 3 then
		   CONCAT('https://pre.ia.diligrp.com:8381/refundOrder/view.action?orderCode=', temp.order_code, '&businessType=', temp.business_type)
...
INSERT INTO settle_order_link(`settle_order_id`, `type`, `url`)
	...
	case temp.business_type
		when 1 then
			CONCAT('https://pre.ia.diligrp.com:8381/api/leaseOrder/queryPrintData?orderCode=', temp.order_code, '&businessType=', temp.business_type)
		when 2 then
			CONCAT('https://pre.ia.diligrp.com:8381/api/earnestOrder/queryPrintData?orderCode=', temp.order_code, '&businessType=', temp.business_type)
		when 3 then
		   CONCAT('https://pre.ia.diligrp.com:8381/api/depositOrder/queryPrintData?orderCode=', temp.order_code, '&businessType=', temp.business_type)
		end
    FROM  settle_order temp WHERE temp.`type` = 1 AND temp.business_type IN (1,2,3) AND temp.reverse = 0;
...
INSERT INTO settle_order_link(`settle_order_id`, `type`, `url`)
	...
	case temp.business_type
		when 1 then
			CONCAT('https://pre.ia.diligrp.com:8381/api/refundOrder/queryPrintData?orderCode=', temp.order_code, '&businessType=', temp.business_type)
		when 2 then
			CONCAT('https://pre.ia.diligrp.com:8381/api/refundOrder/queryPrintData?orderCode=', temp.order_code, '&businessType=', temp.business_type)
		when 3 then
		   CONCAT('https://pre.ia.diligrp.com:8381/api/refundOrder/queryPrintData?orderCode=', temp.order_code, '&businessType=', temp.business_type)
...
INSERT INTO settle_order_link(`settle_order_id`, `type`, `url`)
	...
	temp.id,
	3,
	replace(temp.return_url, 'ia.diligrp.com', 'pre.ia.diligrp.com')
