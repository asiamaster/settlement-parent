 ...
 */
@Component
public class RefundDispatcher implements SettleDispatcher {
    ...
    @PostConstruct
    public void init() {
        for (RefundService validateService : serviceList) {
     ...
     * @return
     */
    private RefundService determineService(Integer way) {
        RefundService service = serviceMap.get(way);
        if (service == null) {
